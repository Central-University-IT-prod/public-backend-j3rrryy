...
> [!NOTE]
> В продакшен версии `.env` не будет. Поэтому перед запуском бота его будет необходимо создать на основе файла `examples/.env.example`
...
- :construction_worker: **Запуск тестов**

```shell
docker exec -it travel_bot sh
pytest tests
```
